...
var SquareChessGame = {
	createNew: function(area,cell_size, cell_img_size,  box_img_size, b_img_w, b_img_h, chess_img_size, img_dir, bg_color, b_offset_x, b_offset_y){
		var game = {};

		/***********************************\
		 * 图片文件配置
		 \***********************************/
		box_img_file = img_dir + "/box/box.png";
		box2_img_file = img_dir + "/box/box2.png";
		 ...
		 * 界面元素自适应 
		 \***********************************/
		// 自适应游戏界面 
		game.w = b_img_w * cell_size / cell_img_size ;
		game.h = b_img_h * cell_size / cell_img_size ;
		...
		game.hide = function(chess){
			if(chess !=null && chess.sprite != null){ 
				chess.sprite.z = -1;	
				chess.sprite.x = game.w+1;
				chess.sprite.y = game.h+1;
				chess.sprite.tx = game.w+1;
				chess.sprite.ty = game.h+1;
			}
		}
		...
		NOT_GAMEOVER = -2
			...
			// =-1：游戏平局
			// =-2：游戏尚未结束
			game.checkWin = function() { var winner = {}; winner.id = -2; return winner;}

		/***********************************\
		 * 移动规则接口 
		 \***********************************/
		//检查在game的当前状态下，从step.from位置到step.to位置是否合法
		game.checkMove = function(step) { return true;}
		...
		game.startMove = function(){
			if(game.checkMove(game.step) ==  true){
				...
				players[now_player_id].moveOnce(game);
			}
		}
		...
		game.endMove = function(){
			game.updateChess();
			now_player_id++;
			if(now_player_id>=players.length) now_player_id = 0;
			players[now_player_id].moveOnce(game);
